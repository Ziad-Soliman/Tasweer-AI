...
const AIColoringBookGenerator: React.FC<MiniAppProps> = ({ onBack }) => {
    ...
    const handleGenerate = async () => {
        ...
        setResults([]);
        try {
            const images = await geminiService.generateColoringBookPage(description);
            setResults(images.map(base64 => `data:image/png;base64,${base64}`));
        } catch (e) {
            setError(e instanceof Error ? e.message : "Failed to generate coloring pages.");
        } finally {
            setIsLoading(false);
        }
    };
    ...
    return (
        <MiniAppLayout
            title={t('coloring-book-generator-title')}
            description={t('coloring-book-generator-desc')}
            onBack={onBack}
        >
            <div className="max-w-4xl mx-auto flex flex-col gap-8">
                <div className="bg-card border p-6 rounded-lg flex flex-col gap-4">
                     <div>
                        <label className="block text-sm font-medium text-foreground mb-1.5">{t('coloringPageTheme')}</label>
                        <textarea
                            value={description}
                            onChange={(e) => setDescription(e.target.value)}
                            placeholder={t('coloringPageThemePlaceholder')}
                    ...
                    >
                        ...
                        <span>{isLoading ? t('generating') : t('generateColoringPage')}</span>
                    </button>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            {results.map((src, index) => (
                                <img
                                    ...
                                    className="w-full aspect-[4/5] object-cover bg-white rounded-lg border"
                                />
                            ))}
                        </div>
                    </div>
