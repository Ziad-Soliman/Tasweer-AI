...
const AICharacterConceptGenerator: React.FC<MiniAppProps> = ({ onBack }) => {
    ...
    return (
        <MiniAppLayout
            title={t('character-concept-generator-title')}
            description={t('character-concept-generator-desc')}
            onBack={onBack}
        >
            <div className="max-w-4xl mx-auto flex flex-col gap-8">
                <div className="bg-card border p-6 rounded-lg grid md:grid-cols-2 gap-6 items-start">
                    <div className="flex flex-col gap-4 md:col-span-2">
                        <label className="block text-sm font-medium text-foreground">{t('characterDescription')}</label>
                        <textarea
                            value={description}
                     ...
                     <div className="flex flex-col gap-4">
                        <label className="block text-sm font-medium text-foreground">{t('characterStyle')}</label>
                         <select
                            value={style}
                ...
                {isLoading && (
                    <div className="text-center text-muted-foreground">
                        ...
                        <p className="mt-2">{t('generatingConcepts')}...</p>
                    </div>
                )}
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            {results.map((src, index) => (
                                <img
                                    ...
                                    className="w-full aspect-[9/16] object-cover bg-muted rounded-lg border"
                                />
                            ))}
                        </div>
                    </div>
...
export default AICharacterConceptGenerator;
