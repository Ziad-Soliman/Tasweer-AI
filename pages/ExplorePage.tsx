        ...
        <main className="flex-1 flex flex-col items-center p-4 sm:p-6 md:p-8 overflow-y-auto">
            <div className="w-full max-w-3xl">
                <div className="text-center mb-8 animate-fade-in">
                    ...
                    <h1 className="text-4xl sm:text-5xl font-bold tracking-tighter text-foreground">Explore</h1>
                    <p className="text-muted-foreground mt-2 max-w-md mx-auto">Ask anything. Get answers grounded in real-time information from Google Search.</p>
                </div>
                ...
                <form onSubmit={handleFormSubmit} className="relative mb-8 animate-fade-in" style={{ animationDelay: '100ms' }}>
                    ...
                    <input
                        ...
                        placeholder="Ask about recent events, facts, or any topic..."
                ...
                {isLoading && (
                    <div className="flex flex-col items-center justify-center text-center p-8">
                        ...
                        <p className="mt-4 font-semibold text-foreground">Searching the web and generating answer...</p>
                    </div>
                )}
                     ...
                     <div className="animate-fade-in bg-card border border-border rounded-lg p-6">
                        ...
                        {result.sources.length > 0 && (
                            <div className="mt-6 border-t border-border pt-4">
                                <h3 className="text-sm font-semibold uppercase text-muted-foreground mb-3">Sources</h3>
                ...
                {!isLoading && !result && !error && (
                    <div className="text-center text-muted-foreground animate-fade-in" style={{ animationDelay: '200ms' }}>
                        <p className="font-semibold mb-4">Try one of these:</p>
                        <div className="flex flex-wrap justify-center gap-2">
                            <button onClick={() => handleSuggestionClick("Who won the latest F1 race?")} className="px-3 py-1.5 bg-card border border-border rounded-full text-sm hover:border-primary hover:text-primary">Latest F1 race winner</button>
                            <button onClick={() => handleSuggestionClick("What are the top AI trends in 2024?")} className="px-3 py-1.5 bg-card border border-border rounded-full text-sm hover:border-primary hover:text-primary">Top AI trends 2024</button>
                            <button onClick={() => handleSuggestionClick("Summarize the most recent news about space exploration.")} className="px-3 py-1.5 bg-card border border-border rounded-full text-sm hover:border-primary hover:text-primary">Recent space exploration news</button>
                        </div>
                    </div>
                )}
            </div>
